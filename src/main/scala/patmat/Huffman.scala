...
object Huffman {
   ...
   */
  def until(unique: List[CodeTree] => Boolean, comb: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (unique(trees)) trees
    else until(unique, comb)(comb(trees))
  }
  /**
